...
function get_city(t,m){
	...
	document.getElementById("dlqy").value=m;
	document.getElementById("dlqyid").value=parent_id;
}
...
// 获取最后一级乡镇
function get_twon(t,m){
    var parent_id = $(t).val();
    if(!parent_id > 0){
        return;
    }
    var url = '/index.php?m=Admin&c=Api&a=getRegion&level=4&parent_id='+ parent_id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
...
function GetUploadify2s(num,elementid,path,callback)
{       
    var upurl ='/index.php?m=Homedls&c=Uploadify&a=upload&num='+num+'&input='+elementid+'&path='+path+'&func='+callback;
    var iframe_str='<iframe frameborder="0" ';
    iframe_str=iframe_str+'id=uploadify ';          
    iframe_str=iframe_str+' src='+upurl;
    iframe_str=iframe_str+' allowtransparency="true" class="uploadframe" scrolling="no"> ';
