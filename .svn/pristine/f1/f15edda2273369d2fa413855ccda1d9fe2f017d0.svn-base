...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		// 如果在10和60之前，那么就当作是前台人为设置的参数，用以区分是哪个类型的支付
		// 如果不是以上情况，那么就是取默认的
		if ($timeOut >= 10 && $timeOut<=60)
		{
			$timeOut_2 = intval($timeOut/10);
			$wxconfig = get_config_pay3($timeOut_2);
		}
		else
		{
			if($inputObj->GetTrade_type() == "JSAPI")
			{
				$wxconfig = get_config_pay3(3);
			}
			else
			{
				$wxconfig = get_config_pay3(1);
			}
		}

		//异步通知url未设置，则使用配置文件中的url
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($wxconfig['notify_url']);//异步通知url
		}
	...
	{		
		...
		//curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,TRUE);(这两行是官方给的例子，但报错)
		...
		if($useCert == true){
            ...
            $dir = $_SERVER['DOCUMENT_ROOT'] . "/ThinkPHP/Library/Vendor/Wxpay/cert";
