...
class ShopController extends CommonController
{
    ...
    public function money_wait(){
        ...
        //待返金额 今日的待返金额
        $endYesterday=mktime(0,0,0,date('m'),date('d'),date('Y'))-1;//今日开始时间
        $accountWhere['cre_time'] = array('gt',$endYesterday);
        $accountWhere['type'] = 6;
        $accountWhere['deal_type'] = 2;
        $todayMoney = M('account_log')->where($accountWhere)->sum('amount');
        $todayMoney = $todayMoney ? $todayMoney : 0;
        $this->assign('todayMoney',$todayMoney);

        //待返总金额 全部的待返总金额
        $allWhere['type'] = 6;
        $allWhere['deal_type'] = 2;
        $allMoney = M('account_log')->where($allWhere)->sum('amount');
    ...
    public function all_money(){
        ...
        $accountWhere['cre_time'] = array('BETWEEN',array($beginYesterday,$endYesterday));
        $accountWhere['type'] = 17;
        $accountWhere['deal_type'] = 2;

        $yes = M('account_log')->where($accountWhere)->sum('amount');
        if(empty($yes)){
            $yes = 0;
        }
        ...
        $accountWhere['cre_time'] = array('GT',$beginToday);
        $today = M('account_log')->where($accountWhere)->sum('amount');
        if(empty($today)){
            $today = 0;
        }
    ...
    public function ipo(){
        ...
        $endYesterday=mktime(0,0,0,date('m'),date('d'),date('Y'))-1;//今日开始时间
        $accountWhere['cre_time'] = array('gt',$endYesterday);
        $accountWhere['type'] =18;
        $accountWhere['deal_type'] =11;
        $today = M('account_log')->where($accountWhere)->sum('amount');
        $today = $today ? $today : 0;
        ...
        //总返还金额
        $allWhere['type'] =18;
        $allWhere['deal_type'] =11;
        $all = M('account_log')->where($allWhere)->sum('amount');
        $all = $all ? $all : 0;
        $this->assign('all',$all);
        $this->assign('today',$today);
