...
//			 获取所有未结算收益的订单，然后循环处理收益
			...
			$where_og['isdel'] = 0;
			$where_og['is_profit'] = 0;
			$where_og['order_type'] = 0;
            ...
            $orderWhere["isdel"] = 0;
            $orderWhere['order_type'] = array('NOT IN','1,2,3,4,10,12');
            ...
            //查询售后订单
            $orderWhere['order_status'] = 6;
            $shohou = M('order')->where($orderWhere)->count();//待评论的订单数量
            if($shohou > 9){
                $shohou = 9;
            }
            ...
            $this->assign('shohou',$shohou);


            //将所有未成团的拼团订单自动取消订单并退款
            $groupWhere['group_time_end'] = array('lt',time());
            $groupWhere['order_type'] = 5;
            $groupWhere['order_status'] = 1;//已支付
            $groupList = M('order')->where($groupWhere)->select();
            foreach ($groupList as $k => $v){
                if($v['group_people_num_have'] != $v['group_people_num']){
                    //修改订单状态为售后状态
                    M('order')->where('id='.$v['id'])->setField('order_status',6);//售后状态
                    M('order_goods')->where('id='.$v['order_id'])->setField('order_status',-9);//售后完成
                    $orderGoods = M('order_goods')->where('order_id='.$v['id'])->find();
                    $refund = array(
                        'user_id' => $v['user_id'],
                        'order_id' => $v['id'],
                        ...
                        'goods_thumb' => $orderGoods["goods_thumb"],
                        'goods_price' => $orderGoods["goods_price"],
                        'refund_num' => $orderGoods['goods_num'],
                        'refund_total' => $orderGoods["goods_price"] * $orderGoods['goods_num'] - $v['user_coupon_money'],
                        'shop_id' => $orderGoods["shop_id"],
                        'goods_spec_price_id' => $orderGoods["goods_spec_price_id"],
                        'goods_spec_price_name' => $orderGoods["goods_spec_price_name"],
                        'refund_status' => 8,
                        'refund_type' => 3,
                        'goods_type' => 2,
                        'refund_reason' => '拼团失败，金额返回',
                        'refund_description' => '',
                        'images' => '',
                        'add_time' => time(),
                    );
                    $refund_id = M('order_refund')->add($refund);
                    if($v['is_use_coupon'] > 0){
                        M('order')->where('id='.$v['id'])->setField('is_use_coupon',0);
                        M('order')->where('id='.$v['id'])->setField('user_coupon_id',0);
                        M('coupon_user')->where('id='.$v['user_coupon_id'])->setField('order_id',0);
                        M('coupon_user')->where('id='.$v['user_coupon_id'])->setField('use_time',0);
                        M('coupon_user')->where('id='.$v['user_coupon_id'])->setField('use_date',0);
                    }
                    $OrderIn = new \Admin\Controller\OrderController();
                    $OrderIn->order_real_refund($refund_id,0);
                }
		...
		//如果没有openid就重新获取一下
        if (!$userInfo['openid_temp'] && stristr($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger'))
        {
            $refer = $_SERVER["REQUEST_SCHEME"] . "://" . $_SERVER["HTTP_HOST"] . "/M/User/index";
            session("refer", $refer);
            $this->getCode();
        }
		...
		//查询优惠券数量
        $couponWhere['user_id'] = session('user_id');
        $couponWhere['order_id'] = 0;
        $coupon = M('coupon_user')->where($couponWhere)->count();
	...
	*/
    ...
    public function cz(){
        if(IS_POST){
            ...
            $data["order_type"] = 3;
            $data['openid'] = $userInfo['openid_temp'];
