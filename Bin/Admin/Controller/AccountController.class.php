...
//			$where['order_status'] = array('in', '1,2,3,5,6,8');
            $where['order_type'] = array('neq',12);
            $where['confirm_time'] = array('gt',0);
...
//            $where['order_type'] = 0;
		...
		$orderWhere['pay_time'] = array('gt',$endYesterday);
		$orderWhere['order_type'] = array('in',array(0,12));
		$all_money = M('order')->where($orderWhere)->sum('total_commodity_price');
		$all_money = $all_money ? $all_money : 0;
		$all_coupon = M('order')->where($orderWhere)->sum('user_coupon_money');
		$all_coupon = $all_coupon ? $all_coupon : 0;
		$all_money = $all_money + $all_coupon;
		$this->assign('all_money',$all_money);

        //今日团购订单
        $orderWhere['order_type'] =5;
        $all_group = M('order')->where($orderWhere)->sum('total_commodity_price');
        $all_group = $all_group ? $all_group : 0;
        $this->assign('all_group',$all_group);

        //今日秒杀订单
        $orderWhere['order_type'] =6;
        $all_flash = M('order')->where($orderWhere)->sum('total_commodity_price');
        $all_flash = $all_flash ? $all_flash : 0;
        $this->assign('all_flash',$all_flash);
	...
	public function commodityanalysis()
	{
        ...
        $where_ord_2['isdel'] = 0;
        $where_ord_2['pay_time'] = array("gt", 0); // 今天
        $totalMoney = M('order')->where($where_ord_2)->sum('total_commodity_price');
        //加上优惠券的
        ...
        $totalMoney = $totalMoney + $allCoupon;


		$this->assign('goods', $goods);
		$this->assign('order_money', $totalMoney);
