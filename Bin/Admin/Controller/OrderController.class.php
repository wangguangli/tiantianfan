		...
		if (IS_POST)
		{
			...
			if ($refund_status == 3)
			{
				...
				if($data['refund_type'] == 1){
                    $data_og['order_status'] = $refund_status;
                    M('order')->where('id=' . $data['order_id'])->save(array('order_status' => 3));
                }
			}
	...
	public function order_real_refund($id, $status)
	{
		...
		switch ($order['payment_type'])
		{
            ...
            case 'price':
                ...
                break;
            case 'price_pay':
                $res = $this->price_pay_refund($refund);
                break;
...
//            "\"trade_no\":\"$trade_no\"," .
    ...
    public function price_pay_refund($refund)
    {
        $refund_total = $refund['refund_total'];
    ...
    public function orderDc(){
        ...
        if($order){
            ...
            foreach ($orderList as $k => $v){
                $orderInfo = M('order')->find($v['order_id']);
                ...
                if($orderInfo['order_type'] == 5){
                    $xlsData[$k]['order_type'] = '团购订单';
                }elseif($orderInfo['order_type'] == 6){
                    $xlsData[$k]['order_type'] = '秒杀订单';
                }else{
                    $xlsData[$k]['order_type'] = '普通订单';
                }
                ...
                if($orderInfo['district']){
                    $province = M('region')->find($orderInfo['province_id']);
                    $city = M('region')->find($orderInfo['city']);
                    $district = M('region')->find($orderInfo['district']);
                    $xlsData[$k]['address'] = $province['name'].$city['name'].$district['name'].$orderInfo['address'];
                }else{
                    $xlsData[$k]['address'] = $orderInfo['address'];
                }
    ...
    public function exportExcel($title = array(), $data = array(), $fileName = '', $savePath = './upload/export/', $isDown = true)
    {
        ...
        if ($title) {
            ...
            foreach ($title AS $v) {   //设置列标题
                ...
                $obj->getActiveSheet()->getColumnDimension($cellName[$i])->setWidth(30); //设置宽度
                $i++;
            }
            $_row++;
        }
