...
function order_type0($order_id){
    ...
    foreach ($orderGoods as $k => $v){
        ...
        $money_wait = round($goodsInfo['still_fee_bfb'] * 0.01 * $v['goods_total'],2);//待返积分
        $money_all = round($goodsInfo['service_fee_bfb'] * 0.01 * $v['goods_total'],2);//奖金池金额
        ...
        M('config')->where($where)->setField('val',$all_money);
    }
    $userInfo = M('user')->find($orderInfo['user_id']);
    ...
    if($userInfo['first_leader']){
        ...
        if($data['up_level_by_auto'] == 1){
            //先查询自己所有的下级
            if($top['level'] + 1 < 4){
                $topWhere['first_leader'] = $top['id'];
                $allUser = M('user')->where($topWhere)->select();
                foreach ($allUser as $k => $v){
                    $ids[$k] = $v['id'];
                }
                if($ids){
                    $level = $top['level'] + 1;
                    $data = get_config_user_level($level);
                    $idsWhere['id'] = array('in',$ids);
                    $idsWhere['order_type'] = 0;
                    $idsWhere['pay_status'] = 1;
                    $allNumber = M('order')->where($idsWhere)->count();
                    if($allNumber >= $data['number']){
                        //升级
                        M('user')->where('id='.$top['id'])->setField('level',$data['level_ranking']);
                    }
                }
            }
